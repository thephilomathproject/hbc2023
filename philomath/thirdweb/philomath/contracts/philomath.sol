    ...
    */
    ...
    function publishNewProject(
        ...
        projectIdToProjectData[newProjectId] = ProjectData(
            userId,
            userIdToUserData[userId].nameOfPerson,
            nameOfProject,
            startTimestamp,
            endTimestamp,
            grantAmount,
    ...
    function getUserWithId(
        ...
        return userIdToUserData[userId];
    }

    function getProjectWithId(
        uint256 projectId
    ) external view returns (ProjectData memory) {
        return projectIdToProjectData[projectId];
    }

    function getUserProjectIdsWithUserId(
        uint256 userId
    ) external view returns (uint256[] memory) {
        return userIdToProjectIds[userId];
    }
}
